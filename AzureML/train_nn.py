...
stockInfo = run.input_datasets['stockInfo_v1'] #pd.read_csv('stockInfo_v1.csv',header=[0,1])
#stockInfo.columns = ['ticker','sector','exchange','marketCap'] # already named
...
X_train = pd.concat([X.iloc[start:end, :] for (start, end) in train_ranges]).reset_index(drop=True)
y_train = pd.concat([y.iloc[start:end] for (start, end) in train_ranges]).reset_index(drop=True)

X_test = pd.concat([X.iloc[start:end, :] for (start, end) in val_ranges]).reset_index(drop=True)
y_test = pd.concat([y.iloc[start:end] for (start, end) in val_ranges]).reset_index(drop=True)
